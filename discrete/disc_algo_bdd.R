...
A_dual_bdd = function(x, data, genIntv_n)
{
  ans = matrix(ncol = (2*nrow(genIntv_n)) + 1, nrow = length(x))
...
dual_lp_bdd = function(x_start, data, genIntv_n)
{
  model_dual = list()
  
  model_dual$A = A_dual_bdd( x = x_start, data = data, genIntv_n = genIntv_n)
  
  model_dual$obj  = c(genIntv_n$conf_up, -genIntv_n$conf_low, 1)
  model_dual$modelsense = "min"
  model_dual$rhs = x_start
  model_dual$sense =  rep(">=", length(x_start))
  
  model_dual$lb = c( rep(0, (2*nrow(genIntv_n))), -Inf)
  model_dual$ub = rep(Inf, (2*nrow(genIntv_n)) + 1)
...
primal_lp_bdd =  function(x_start, data, genIntv_n)
{
  model_primal = list()
  
  model_primal$A = t( A_dual_bdd( x = x_start, data = data, genIntv_n = genIntv_n) )
  
  model_primal$obj  = x_start
  model_primal$modelsense = "max"
  model_primal$rhs = c(genIntv_n$conf_up, -genIntv_n$conf_low, 1)
  model_primal$sense =  c(rep("<=",(2*nrow(genIntv_n))), "=")
...
arg_min_bdd = function(lambda, data, genIntv_n , eps = 10^(-7), analyze = F, 
                         low_bd, up_bd)
{
  data_sort = sort(data)
  ...
  A_dual_temp = A_dual_bdd( x = x_temp, 
...
# low_bd1 = -1
# up_bd1 = 1
# x1 = runif(n1, min = low_bd1, max = up_bd1)
# 
# x1_sort = sort(x1)
# 
