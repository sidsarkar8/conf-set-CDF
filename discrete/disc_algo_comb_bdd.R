...
A_dual_comb_bdd = function(x, data, genIntv_n, genCDF_n)
{
  ans = matrix(ncol = (2*nrow(genIntv_n)) + (2*nrow(genCDF_n)) + 1, 
               nrow = length(x))
  ...
  for( i in 1:length(x))
  {
    ...
    ans[i, (2*nrow(genIntv_n)) + (2*nrow(genCDF_n)) + 1] = 1
  }
  return(ans)
}
...
dual_lp_comb_bdd = function(x_start, data, genIntv_n, genCDF_n)
{
  model_dual = list()
  
  model_dual$A = A_dual_comb_bdd( x = x_start, data = data, genIntv_n = genIntv_n,
                                  genCDF_n =  genCDF_n)
  
  model_dual$obj  = c(genIntv_n$conf_up, -genIntv_n$conf_low, 
                      genCDF_n$conf_up, -genCDF_n$conf_low, 1)
  
  model_dual$modelsense = "min"
  model_dual$rhs = x_start
  model_dual$sense =  rep(">=", length(x_start))
  
  model_dual$lb = c( rep(0, (2*nrow(genIntv_n)) + (2*nrow(genCDF_n))), -Inf)
  model_dual$ub = rep(Inf, (2*nrow(genIntv_n)) + (2*nrow(genCDF_n)) + 1)
...
primal_lp_comb_bdd =  function(x_start, data, genIntv_n, genCDF_n)
{
  model_primal = list()
  
  model_primal$A = t( A_dual_comb_bdd( x = x_start, data = data,
  ...
  model_primal$sense =  c(rep("<=",(2*nrow(genIntv_n)) + (2*nrow(genCDF_n))),
                          "=")
...
arg_min_comb_bdd = function(lambda, data, genIntv_n, genCDF_n, eps = 10^(-7), 
                            analyze = F, low_bd, up_bd)
{
  ...
  x_temp = c(x_temp, data_sort[1] - eps,  data_sort[length(data)] + eps, low_bd, up_bd)
  
  
  A_dual_temp = A_dual_comb_bdd( x = x_temp, 
