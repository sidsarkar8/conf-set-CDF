                             ...
                             genCDF_n1, eps = 10^-6)
{
  ...
  n1_trim_low =  floor(log(log(n)))
  n1_trim_up =  floor(n - log(log(n)))
  
  #### starting optimization
  ### sorted data point only sufficient to check
  x1_trim = x1_sort[n1_trim_low:n1_trim_up]
  genCDF_n1_trim = genCDF_n1[n1_trim_low:n1_trim_up,]
  
  x1_start = c(x1_trim, x1_trim + eps)
  
  dual_temp = dual_lp_dw_bdd(x_start = x1_start, data = x1_trim,
                             genCDF_n = genCDF_n1_trim)
...
n2_trim_low =  floor(log(log(n2)))
n2_trim_up =  floor(n2 - log(log(n2)))


up_tail = (sort(x2)[n2_trim_up]/H(sort(x2)[n2_trim_up]))*K2
low_tail = -(abs(sort(x2)[n2_trim_low])/H(sort(x2)[n2_trim_low]))*K2
