...
source("/home/siddhaarth/conf_tail/sims_new/other_methods_subg.R")
...
H = function(x, mu = 0, a = 1){
  
  if(a<0){stop("a is negative")}
  
  return( exp(a*(x-mu)^2) ) }

## returns 2 possible inverses
H_inv = function(x, mu = 0, a = 1){
  
  if(a<0){stop("a is negative")}
  
  return(  sqrt(log(x)/a)*c(-1,1) + mu )
}

H_der = function(x, mu = 0, a = 1){
  
  if(a<0){stop("a is negative")}
  
  return( (2*a*(x-mu))*exp(a*(x-mu)^2) )
}

## returns 2 possible derivative inverses
H_der_inv = function(x, mu = 0, a = 1){
  
  if(a<0){stop("a is negative")}
  
  if(x > 0)
  {
    return(mu + sqrt( lambertWp(x^2/(2*a))/(2*a)))
  }else{
    return(mu - sqrt( lambertWp(x^2/(2*a))/(2*a)))
  }
}
...
# for( i in 1:100)
# {
#   ans[i] = H_inv(H( mu1 + (i-50)/10, 1/10, mu1), 1/10, mu1)[2]
# }

# plot(y = ans, x =  (1:100 - 50)/10 + mu1)
# abline(a = 0, b = 1)
...
A_dual_dw = function(x, data)
{
  ...
  ans = cbind(ans_pos, ans_neg, H(x, 1, mean(data)), rep(1, length(x)))
...
parallel_fin_func = function(simul_idx, n2, genCDF_n2, mu2)
{
  ...
  write.csv(qq1,
            file = paste("/home/siddhaarth/conf_tail/sims_new/subg/indiv_files/subg_",
...

{
  ...
  x2 = rdistr(n2)
  x2 = H_inv(x2^q1,1,0)[1]*(2*rbinom(n2, size = 1, prob = 1/2) -1)
  K2 = 2*q1 + 1
  ...
  #TODO
  qq1 = data.frame("simul_idx" = simul_idx, 
                   "up_dw" = up_dw, 
                   "low_dw" = low_dw,
                   "up_t_test" = CI_t[2], 
                   "low_t_test" = CI_t[1])
  ...
  write.csv(qq1,
            file = paste("/home/siddhaarth/conf_tail/sims_new/subg/subg_",
                         n2_seq[n2_idx],"_",simul_idx,".csv",sep = "" ))
...
{
  print(paste(n2_seq[n2_idx], ": run sims"))
  ...
  df_n2_subg = foreach( simul_idx = 1:M,  .combine = rbind ) %dopar%{
    parallel_fin_func(simul_idx, 
                      n2 = n2_seq[n2_idx],
                      genCDF_n2 = genCDF_n2)
  }
  
  write.csv(df_n2_subg,
            file = paste("/home/siddhaarth/conf_tail/sims_new/subg_",
