...
source("~/Documents/Projects/conf_tail/other_methods/other_methods_x2.R")
...
H_inv = function(x, mu){return(sqrt(x) + mu)}
...
{  
  ...
  while( TRUE)
  {
    ...
    (x1_new = arg_min_end(lambda = lambda_temp,
                          data = x1,
                          genCDF_n = genCDF_n1,
                          low_bd = -multi_factor*H_inv(K1, mean(x1)),
                          up_bd = multi_factor*H_inv(K1, mean(x1)),
                          analyze = F))
...
parallel_fin_func = function(simul_idx, n2, genCDF_n2, mu2)
{
  ...
  x2 = rdistr(n2)
  x2 = H_inv(x2^q1,0)*(2*rbinom(n2, size = 1, prob = 1/2) -1) + mu2
  K2 = 2*q1 + 1
...
x2_100_fin <- x2_100 %>%
  ...
  dplyr::select(
    "width_dw", "width_t_test", "width_catoni", "width_cheby",
    "cov_dw", "cov_t_test", "cov_catoni", "cov_cheby"
  )
