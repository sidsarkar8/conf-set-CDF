### concetration for mean for E[ exp(a(X-mu)^2)] <= 2
## TODO
emp_bern_subg_CI=  function(x1, a, K, alpha = 0.05) {
  n    <- length(x1)
  sigma_hat = sd(x1)
  
  threshold_n = sqrt(2*n*sigma_hat^2*log(2/delta)) + 
    6*sqrt(log(2*n^2*K/alpha)/a)*log(2/alpha)
  
  return( mean(x1) + (threshold_n/n)*c(-1,1)  )
}
